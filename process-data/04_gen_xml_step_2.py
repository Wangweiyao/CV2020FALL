...
def write_multifood_xml(img_name, bb_info2xml, category):
    ...
    for i in range(0, len(bb_info2xml)):
        ...
        folder.text = 'UECFOOD100/' + bb_info2xml[i][0]

        filename = ET.SubElement(root, 'filename')
        filename.text = img_name

        path = ET.SubElement(root, 'path')
        path.text = '/home/weiyao/data/UECFOOD100/' + bb_info2xml[i][0] + '/' + img_name
        ...
        tree.write('/home/weiyao/data/UECFOOD100/' + bb_info2xml[i][0] + '/' + 'annotations_new/' + img_name[:-4] + '.xml', pretty_print=True)


if __name__ == '__main__':
    print('Start here')
    category = read_category()
    with open('/home/weiyao/data/UECFOOD100/multiple_food.txt', 'r') as file:
        for i, line in enumerate(file):
            if i > 0:
                line = line.rstrip('\n')
                line = line.split(' ')[:-1]
                ...
                for j in range(1, len(line)):
                    with open('/home/weiyao/data/UECFOOD100/' + line[j] + '/new_bb_info.txt', 'r') as bb_file:
